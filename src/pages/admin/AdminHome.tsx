...
import {
  ResponsiveContainer, PieChart, Pie, Cell, Tooltip, LineChart, Line, XAxis, YAxis,
} from "recharts";
...
const gradientCards = [
  { key: "users", label: "Total Users", icon: Users, gradient: "from-blue-500/20 to-blue-600/5", iconBg: "bg-blue-500/20", iconColor: "text-blue-400" },
  { key: "forecasts", label: "Active Forecasts", icon: TrendingUp, gradient: "from-emerald-500/20 to-emerald-600/5", iconBg: "bg-emerald-500/20", iconColor: "text-emerald-400" },
  { key: "likes", label: "Likes Today", icon: Heart, gradient: "from-pink-500/20 to-pink-600/5", iconBg: "bg-pink-500/20", iconColor: "text-pink-400" },
  { key: "messages", label: "Open Messages", icon: MessageSquare, gradient: "from-purple-500/20 to-purple-600/5", iconBg: "bg-purple-500/20", iconColor: "text-purple-400" },
];
...
export default function AdminHome() {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel("admin-home-rt")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "forecasts" }, loadStats)
  ...
  const loadAll = async () => {
    await Promise.all([loadStats(), loadChartData(), loadTopUsers(), loadDonut(), loadMessages(), loadEvents(), loadSchedule()]);
    setLoading(false);
  };
  ...
  const loadChartData = async () => {
    ...
    const data = await Promise.all(days.map(async (date) => {
      ...
      return { name: new Date(date).toLocaleDateString("en", { weekday: "short" }), users: ur.count || 0, forecasts: fr.count || 0 };
    }));
    setChartData(data);
  };
  ...
  const statsValues: Record<string, number> = {
    users: stats.users, forecasts: stats.forecasts, likes: stats.likes, messages: stats.messages,
  };
  const yesterdayValues: Record<string, number> = {
    users: yesterdayStats.users, forecasts: yesterdayStats.forecasts, likes: yesterdayStats.likes, messages: yesterdayStats.messages,
  };
  ...
  return (
    <>
      ...
      <motion.div variants={container} initial="hidden" animate="visible" className="space-y-6">
          ...
          {gradientCards.map((card) => {
            ...
            return (
              <motion.div key={card.key} variants={item} whileHover={{ y: -4, transition: { duration: 0.2 } }}>
                <Card className={`bg-gradient-to-br ${card.gradient} border-border/50 hover:shadow-lg transition-shadow cursor-pointer`}>
        ...
        {/* Charts row: 7-Day Trends + Platform Overview */}
              ...
              <CardContent className="space-y-3">
                ...
                {recentMessages.map((msg, i) => (
                  ...
                  >
                    ...
                    <Badge variant={msg.priority === "high" ? "destructive" : "secondary"} className="text-[10px] shrink-0">
                      {msg.priority || "normal"}
                    </Badge>
                  </motion.div>
                ))}
        ...
        <motion.div variants={item}>
          <Card>
            ...
            <CardContent className="flex gap-3 flex-wrap">
              <Link to="/admin/notifications"><Button size="sm" className="gap-1"><Send className="w-3.5 h-3.5" /> Send Notification</Button></Link>
              <Link to="/admin/analytics"><Button size="sm" variant="secondary" className="gap-1"><TrendingUp className="w-3.5 h-3.5" /> View Reports</Button></Link>
              <Link to="/admin/content"><Button size="sm" variant="outline" className="gap-1">Manage Content</Button></Link>
              <Link to="/admin/live-stream"><Button size="sm" variant="outline" className="gap-1"><Radio className="w-3.5 h-3.5" /> Live Stream</Button></Link>
            </CardContent>
          </Card>
        </motion.div>
      </motion.div>
